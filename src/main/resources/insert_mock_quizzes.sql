...
-- 1. Sửa lỗi thiếu username
UPDATE users
SET username = 'lecturer_mock'
WHERE email = 'lecturer_mock@test.com';

-- 2. Cấp quyền Giảng viên (MENTOR)
DECLARE @role_mentor_id BIGINT;
SELECT @role_mentor_id = id FROM roles WHERE name = 'MENTOR';

IF NOT EXISTS (SELECT 1 FROM user_roles WHERE user_id = (SELECT id FROM users WHERE email = 'lecturer_mock@test.com') AND role_id = @role_mentor_id)
BEGIN
INSERT INTO user_roles (user_id, role_id)
SELECT id, @role_mentor_id FROM users WHERE email = 'lecturer_mock@test.com';
END
